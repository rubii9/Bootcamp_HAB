...
-- también le podríamos llamar project_employees por ejemplo
-- como es una relación M:N necesitamos las claves foráneas a ambas tablas (employee y project)
...
alter table project change column init_date start_date date not null;
select * from project;
...
select distinct p.* from project p, assignments a where a.id_employee = 7521;
